...
## Situation
사내 소셜 네트워크 서비스 Yammer의 유저 인게이지먼트 대시보드에서 8월 한 달간 주간 활성 사용자(WAU)가 지속적으로 감소하는 현상이 관찰되었습니다.
{% include framework/shortcodes/figure.html src="/assets/images/gen/blog/WAU.png" alt="WAU" %}

## Task
사용자 이벤트 로그 데이터를 분석하여 사용자 활동 패턴과 문제의 원인을 파악하고 서비스 개선을 위한 인사이트를 도출하는 것이 목표였습니다.

## Action

- WAU 감소 원인 가설 수립
- 신규 가입자 및 신규 가입자의 활성 추이 분석 (Growth 검증)
- 코호트 분석을 통한 기존 사용자 리텐션 변화 확인
- 디바이스별 WAU 분석
- 이메일 리인게이지먼트 퍼널 분석

## Result

- WAU 감소 원인이 신규 사용자 감소가 아닌 기존 사용자 참여율 저하임을 확인
- 모바일 환경에서 사용자 참여 감소가 두드러지는 것을 발견
- 이메일 리인게이지먼트 퍼널에서 사용자 재참여를 유도하는 주간 요약 리포트 이메일 내 클릭률(CTR)이 감소하고 있음을 확인
- 분석 결과를 바탕으로 서비스 개선을 위해 모바일 앱 점검 및 이메일 링크/랜딩 경험 검증을 우선순위로 제안
